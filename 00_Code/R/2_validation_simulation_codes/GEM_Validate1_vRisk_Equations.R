...
calculate_adjusted_risk <- function(cond, inc_probs, matched_params, pd, living_idx,
  ...
  #' risk factors: cvd, obesity, t2d
    ...
    if (cohort_female == 1) {
      cvd_multiplier[] <- cvd_rr_stroke_bysex$value[cvd_rr_stroke_bysex$female == 1][1]
    } else if (cohort_female == 0) {
      cvd_multiplier[] <- cvd_rr_stroke_bysex$value[cvd_rr_stroke_bysex$female == 0][1]
      }
  ...
  #' risk factors: obesity, t2d, ckd, cvd 
    ...
    ## 3) cvd ----
      ...
      if (length(cvd_idx) > 0)  {
        ...
        non_cvd_death <- inc_ac_death - death_cvd
        cvd_multiplier[cvd_idx] <- inc_ac_death[living_idx][cvd_idx] / non_cvd_death[living_idx][cvd_idx]
      }
    ...
    ## 4) ckd ---- 
      ...
      if (length(ckd_idx) > 0){
        ...
        inc_ac_death <- matched_params[["incidence_ac_death_val"]]
        # Compute non-ckd death incidence:
        non_ckd_death <- inc_ac_death - death_ckd
        ckd_multiplier[ckd_idx] <- inc_ac_death[living_idx][ckd_idx] / non_ckd_death[living_idx][ckd_idx]
      }
